...
def ingr_search(query, series):
    ...
    return (IngrTable(items, html_attrs={'align':'center', 'class': 'table table-hover table-sm'}))
...
def load_list_table(filename):
    ...
    return (ListTable(items, html_attrs={'align':'center', 'class': 'table table-hover table-sm'}))

def load_groc_list(lists):
    list_of_lists = []
    for list in lists:
        list_of_lists.append(GrocListTableItem(list.list_name, '#', '#'))
    return (GrocListTable(list_of_lists, html_attrs={'align':'center', 'class': 'table table-hover table-sm'}))
